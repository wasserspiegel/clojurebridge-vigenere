(ns clojurebridge-cipher-crack.util)
...
(defn mat-freq-var
  ...
  [rf s]
   ...
   ;  freqs-by-i
     (for [cy alphabeth] 
       (vec (for [x is] (fch (get rf cy) (get (get freqs-by-i x) cy)))))
     ))
...
(defn keys-and-avg 
  [mat] 
  (sort-by second < (mapv vec (partition 2 
                     (interleave (mapv identity alphabeth) (mapv #(apply + %) (transpose mat)))
))))
...
(defn candidates
  ...
  (vec (for [cl cls] 
    (let [chks (chunks-by-cipher-len cl s)
          cands (for [pos (range cl)] (keys-and-avg (mat-freq-var eng (nth chks pos))))]
        {:cipher-len cl
